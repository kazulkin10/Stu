...
function renderDays(){ if(!daysEl) return; const days = Math.floor((nowMSK_utc_ms() - FRIENDSHIP_START_MSK_UTC)/86400000); daysEl.textContent = days.toLocaleString('ru-RU'); }
...
function setTile(tile, value){
  ...
  setTimeout(()=>{ top.textContent = next; }, 380);
}
const pad = (v, isDays)=> (isDays && v>=100) ? String(v).padStart(3,'0') : String(v).padStart(2,'0');
...
function updateRow(rowId, spec, ringsPrefix){
  ...
  const target = targetMSK_utc_ms(spec.m, spec.d);
  ...
  const vals = [pad(d,true), pad(h), pad(m), pad(sec)];
  vals.forEach((v, i)=> setTile(tiles[i], v));

  setRing(ringsPrefix+'H', m/60);
  setRing(ringsPrefix+'M', sec/60);
...
function ringTemplate(ratio){
  ...
  return `<svg viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg" role="img">
    ...
    <circle cx="${c}" cy="${c}" r="${r}" fill="none" stroke="rgba(255,255,255,.1)" stroke-width="10"/>
...
// ---------- 20s-заставка по клику на логотип ----------
